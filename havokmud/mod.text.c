...
char *tfd_main_menu =  "    1) File to edit\n\r"
					   "    2) View old file\n\r"
...
void TfdEdit(struct char_data *ch, char *arg)
{
	if(ch->specials.tfd == TFD_MAIN_MENU) {
		...
		switch(atoi(arg)) {
			case 0: UpdateTfdMenu(ch);
					return;
			case 1: ch->specials.tfd = CHANGE_TFD_FILE;
					ChangeTfdFile(ch, "", ENTER_CHECK);
					return;
			case 2:	if(ch->specials.txtedit->file > 0 && ch->specials.txtedit->file < 4) {
                   ...
                   return;
			case 3:	if(ch->specials.txtedit->file > 0 && ch->specials.txtedit->file < 4) {
                   ...
                   return;
			case 4: ch->specials.tfd = CHANGE_TFD_BODY;
					ChangeTfdBody(ch, "", ENTER_CHECK);
					return;

			case 8:
				ch->desc->connected = CON_PLYNG;
				act("$n has returned from editing a textfile.", FALSE, ch, 0, 0, TO_ROOM);
...
int write_txt_to_file(struct char_data *ch)
{
	...
	switch(tfd->file) {
		case 1: sprintf(buf,"%s", NEWS_FILE); break;
		case 2: sprintf(buf,"%s", MOTD_FILE); break;
		case 3: sprintf(buf,"%s", WMOTD_FILE); break;
		default:
...
void ViewOldTfd(struct char_data *ch, char *arg, int type)
{
	char buf[255], buffer[254], contents[MAX_STRING_LENGTH];

	switch(ch->specials.txtedit->file) {
		case 1: file_to_string(NEWS_FILE, contents);sprintf(buf,"news"); break;
		case 2: file_to_string(MOTD_FILE, contents);sprintf(buf,"motd"); break;
		case 3: file_to_string(WMOTD_FILE, contents);sprintf(buf,"wizmotd"); break;
		default:
			send_to_char("No valid filetype selected.\n\r",ch);
			ch->specials.tfd = TFD_MAIN_MENU;
			UpdateTfdMenu(ch);
...
void ChangeTfdDate(struct char_data *ch, char *arg, int type)
{
	...
	if(type != ENTER_CHECK) {
		if(tfd->date)
			free(tfd->date);
		switch(tfd->file) {
			case 1: sprintf(buf,"$c000W[$c000BNews for %s$c000W]$c000w", arg);
					break;
			case 2: sprintf(buf,"$c000W[$c000BMessage of the Day, %s$c000W]$c000w", arg);
					break;
			case 3: sprintf(buf,"$c000W[$c000BWizard's Message of the Day, %s$c000W]$c000w", arg);
					break;
			default:
				send_to_char("No valid filetype selected.\n\r",ch);
				ch->specials.tfd = TFD_MAIN_MENU;
	...
	sprintf(buf, "Current file header: %s", tfd->date?tfd->date:"None specified");
	send_to_char(buf, ch);
	send_to_char("\n\r\n\rPlease enter current date (for instance: March 24, 2003): ", ch);
