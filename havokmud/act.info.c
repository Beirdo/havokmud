...
void do_score(struct char_data *ch, char *argument, int cmd)  {
 ...
 dlog("in do_score");
  ...
  ch_printf(ch, "%sYour alignment is: %s%s\n\r", color, color2,AlignDesc(GET_ALIGNMENT(ch)),color);
	...
	if ( !(GetMaxLevel(ch)>MAX_MORT || (IS_NPC(ch) && !IS_SET(ch->specials.act,ACT_POLYSELF)))) {
		...
		for (x=0; x < MAX_CLASS; x++) {
  			if (HasClass(ch, pc_num_class(x))) {
    			...
    			sprintf(buf2, "%s     Level: %s%2d  %-15s%s",color, color2, GET_LEVEL(ch, x), class_names[x], color);
    			strcat(buf, buf2);
    			sprintf(buf2,"Exp needed: %s%s\n\r", color2,formatNum((titles[x][GET_LEVEL(ch, x)+1].exp)- GET_EXP(ch)));
    			strcat(buf,buf2);
  			}
		}
  		send_to_char(buf,ch);
	...
	ch_printf(ch,"%sYour main character class is a %s%s%s.\n\r", color, color2, class_names[ch->specials.remortclass-1], color);
...
*/
	...
	if (IS_SET(ch->specials.affected_by2,AFF2_WINGSBURNED)) {
	   	send_to_char("$c0009Your burned and tattered wings are a source of great pain.\n\r",ch);
	}
 	...
 	extern char *RaceName[];
		...
		case 's':
		  if (GET_STR(person) < 18)
		    sprintf(buffer,"%s [STR:%-2d  INT:%-2d  WIS:%-2d  CON:%-2d  DEX:%-2d  CHR:%-2d] ", GET_NAME(person),
		       GET_STR(person),GET_INT(person),GET_WIS(person),
		       GET_CON(person),GET_DEX(person),GET_CHR(person));
		  else
		    sprintf(buffer,"%s [STR:%-2d(%1d)  INT:%-2d  WIS:%-2d  CON:%-2d  DEX:%-2d  CHR:%-2d] ", GET_NAME(person),
 ...
 void do_who(struct char_data *ch, char *argument, int cmd) {
  ...
  char levelimm[127], immbuf[127];
	...
	if( IS_IMMORTAL(ch) ) {  //Title
		ch_printf(ch,"%s         Havok Players [God Version | 'who -?' for help]\n\r"
				       "         -----------------------------------------------\n\r",color);
	} else if(cmd==234) {
	...
	for (i = 1, spl = 0; i <= MAX_EXIST_SPELL; i++, spl++) {
		if (GetMaxLevel(ch) > LOW_IMMORTAL ||
			...
			if (!spells[spl]) {
				...
				sprintf(buf + strlen(buf),
				"[%3d] %-30s  <%3d> %2d %3d %3d %3d %3d %3d %3d %3d %3d\n\r",
				i, spells[spl],
		...
		while(thiefskills[i].level != -1) {
			sprintf(buf,"[%-2d] %-30s %-15s",thiefskills[i].level, thiefskills[i].name,
				/* Start Change Jan 22, 2004 Odin
				   Changed thief skills to only show steal with default level based on dex */
...
void do_clanlist(struct char_data *ch, char *arg, int cmd)
{
	...
	char buf[254], name[MAX_STRING_LENGTH];
	char saintbuf[MAX_STRING_LENGTH], leaderbuf[MAX_STRING_LENGTH], memberbuf[MAX_STRING_LENGTH];
	char saints[MAX_STRING_LENGTH], leaders[MAX_STRING_LENGTH], members[MAX_STRING_LENGTH];
	...
	if(!*arg) {
		...
		if (!isdigit(*arg)) {
			...
			if(clan < 1 || clan > MAX_CLAN) {
				...
				for(i=0;i<top_of_p_table+1;i++) {
					if (load_char((player_table + i)->name, &player) > -1) {
						...
						if(GET_CLAN(tmp) == clan) {
							if(IS_IMMORTAL(tmp)) {
								sprintf(saintbuf, "$c000c[$c0008%s$c000c] $c000w%s\n\r",
										(GET_SEX(tmp)?((GET_SEX(tmp) != SEX_FEMALE)?"Patron":"Matron"):"Notron"),
										tmp->player.title?tmp->player.title:GET_NAME(tmp));
								strcat(saints,saintbuf);
							} else if (IS_SET(tmp->specials.act, PLR_CLAN_LEADER)) {
								sprintf(leaderbuf, "$c000c[$c0008Leader$c000c] $c000w%s $c000w[%ld]\n\r",
										tmp->player.title?tmp->player.title:GET_NAME(tmp), CalcPowerLevel(tmp));
								strcat(leaders,leaderbuf);
							} else { /* just a member, apparently */
								sprintf(memberbuf, "$c000c[$c0008Member$c000c] $c000w%s\n\r",
										tmp->player.title?tmp->player.title:GET_NAME(tmp));
				...
				/* may have to append these to string blocks, if clans (or titles!?) get real big */
