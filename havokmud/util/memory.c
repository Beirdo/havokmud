...
void *memalloc( int size )
{
	...
	frag = memoryFragmentFindBySize(&fragmentFreePool, size);
	if( frag ) {
		return( frag );
	}

	/* Next check the deferred pool */
	frag = memoryFragmentFindBySize(&fragmentDeferPool, size);
	if( frag ) {
		return( frag );
	}
						   ...
						   Locked_t locked )
{
	...
	if( item ) {
		...
		item2 = BalancedBTreeFind( tree, &fragment->start, UNLOCKED, FALSE );
		if( item2 ) {
