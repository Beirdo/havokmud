...
int CalcThaco(struct char_data *ch)
{
 ...
 if (IS_AFFECTED2(ch,AFF2_PROTECT_FROM_GOOD)) { /* get +1 to hit good */
    if (ch->specials.fighting) {
	if (IS_GOOD(ch->specials.fighting)) calc_thaco -=1;
    }
  }/* end prot good check */

 if (IS_AFFECTED(ch,AFF_PROTECT_FROM_EVIL)) { /* get +1 to hit evil */
    if (ch->specials.fighting) {
	if (IS_EVIL(ch->specials.fighting)) calc_thaco -=1;
			...
			/* prot from evil */
if (ch->specials.fighting) {
 if (IS_AFFECTED(ch->specials.fighting,AFF_PROTECT_FROM_EVIL)) {
     if(IS_EVIL(ch))
	calc_thaco+=4;
  }
			...
			/* prot from evil */
if (ch->specials.fighting) {
 if (IS_AFFECTED2(ch->specials.fighting,AFF2_PROTECT_FROM_GOOD)) {
     if(IS_GOOD(ch))
	calc_thaco+=4;
  }
...
struct char_data *FindVictim( struct char_data *ch)
{
					...
					if (!in_group(ch, tmp_ch)) {
						...
						if (!IS_NPC(tmp_ch)) {
							...
							if(HasClass(tmp_ch, CLASS_WARRIOR|CLASS_BARBARIAN|CLASS_PALADIN|CLASS_RANGER))
								...
								ctot++;
							else if (HasClass(tmp_ch,CLASS_MAGIC_USER) || HasClass(tmp_ch,CLASS_SORCERER))
								mtot++;
  ...
  /*
    ...
    */
	...
	total = (fjump*ftot)+(cjump*ctot)+(tjump*ttot)+(njump*ntot)+(mjump*mtot)+(djump*dtot)+(kjump*ktot);
  ...
  default:
    ...
    break;
  	...
  	if((Our_Bit == IMM_PIERCE) && (ch->specials.remortclass == BARBARIAN_LEVEL_IND + 1))
		dam = dam - (int) (dam*GET_LEVEL(ch,BARBARIAN_LEVEL_IND))/200; // 25% discount on damage taken.
...
if(IS_SET(ch->immune, IMM_FIRE))
  return;
if(IS_SET(ch->M_immune, IMM_FIRE))
  if(dice(1,2) > 1)
   return;
...
if(GET_RACE(ch) == RACE_AVARIEL)   /*Races with fire prone wings*/
 ...
 towards the ground!",ch);
          act("$n's wings can't support him and he plummets towards\
...
int FSkillCheck(struct char_data *ch, int fskill)
{
	...
	fskill = fskill + 298; /* convert from bit to skill def */
	perc = number(1,110);
		...
		if (ch->skills[fskill].learned > 75 && ch->skills[fskill].learned < max) { /* only learn if skill is high enough */
			if (/*number(1, 96) > ch->skills[fskill].learned  &&*/ (number(1,10) == 1)) {
				...
				if (ch->skills[fskill].learned >= max) {
					send_to_char("You are now a master of fighting in your current style!\n\r", ch);
				}
		    }
		}
		return(FALSE);
