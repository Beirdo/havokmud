...
int necromancer(struct char_data *ch, int cmd, char *arg, struct char_data *mob, int type)
{
	...
	if ((GET_HIT(ch) < (GET_MAX_HIT(ch) / 4)) && ch->specials.fighting) {
		/* ouch, we are.. let's see if we can get outta here */
			if(!number(0,2)) { /* cool, we're outtie! */
	...
	if (!ch->specials.fighting) {
		if((GET_HIT(ch) < GET_MAX_HIT(ch)-10) && (GetMaxLevel(ch) >= 10)) {
			/* timer to heal up! */
		...
		if (!ch->desc) { /* is it a mob? */
			...
			if (!affected_by_spell(ch,SPELL_SUIT_OF_BONE)) {
				act("$n utters the words 'bones of the dead'",FALSE,ch,0,0,TO_ROOM);
				cast_suit_of_bone(GetMaxLevel(ch),ch,"",SPELL_TYPE_SPELL,ch,0);
				return(TRUE);
			}
			if (!affected_by_spell(ch,SPELL_PROTECT_FROM_GOOD) && IS_EVIL(ch)) {
				act("$n traces a protective rune in the air.",FALSE,ch,0,0,TO_ROOM);
				cast_protection_from_good(GetMaxLevel(ch),ch,"",SPELL_TYPE_SPELL,ch,0);
				return(TRUE);
			}
			if (!affected_by_spell(ch,SPELL_SPECTRAL_SHIELD)) {
				act("$n utters the words 'steel of the legions'",FALSE,ch,0,0,TO_ROOM);
				cast_spectral_shield(GetMaxLevel(ch),ch,"",SPELL_TYPE_SPELL,ch,0);
				return(TRUE);
			}
			if (!affected_by_spell(ch,SPELL_GLOBE_DARKNESS) && !IS_AFFECTED(ch,AFF_DARKNESS)) {
				act("$n utters the words 'home sweet home'",FALSE,ch,0,0,TO_ROOM);
...
//				}
				...
				if (!affected_by_spell(ch,SPELL_ENDURE_COLD)) {
					act("$n utters the words 'no more arthritis'",FALSE,ch,0,0,TO_ROOM);
					cast_endure_cold(GetMaxLevel(ch),ch,"",SPELL_TYPE_SPELL,ch,0);
					return(TRUE);
				}
				if (!affected_by_spell(ch,SPELL_GLOBE_MINOR_INV)) {
			...
			if (GetMaxLevel(ch)>31) {
				if (!affected_by_spell(ch,SPELL_GLOBE_MAJOR_INV)) {
					act("$n utters the words 'protect, I said!'",FALSE,ch,0,0,TO_ROOM);
					cast_globe_major_inv(GetMaxLevel(ch),ch,"",SPELL_TYPE_SPELL,ch,0);
					return(TRUE);
				}
				if (!affected_by_spell(ch,SPELL_STONE_SKIN)) {
					act("$n utters the words 'tomb stone'",FALSE,ch,0,0,TO_ROOM);
					cast_stone_skin(GetMaxLevel(ch),ch,"",SPELL_TYPE_SPELL,ch,0);
					return(TRUE);
				}
			}
			...
			if (GetMaxLevel(ch)>40) {
				if (!affected_by_spell(ch,SPELL_VAMPIRIC_EMBRACE)) {
					act("$n utters the words 'kiss of death'",FALSE,ch,0,0,TO_ROOM);
					cast_vampiric_embrace(GetMaxLevel(ch),ch,"",SPELL_TYPE_SPELL,ch,0);
					return(TRUE);
				}
			}
			...
			if (GetMaxLevel(ch) > 20) {
				if (affected_by_spell(ch,SPELL_POISON) || affected_by_spell(ch,SPELL_DISEASE) || affected_by_spell(ch,SPELL_DECAY)) {
			...
			if (GET_MOVE(ch) < GET_MAX_MOVE(ch)/2) {
				act("$n utters the words 'feet of the wight'",FALSE,ch,0,0,TO_ROOM);
...

	{
		...
		switch(lspell) {
			...
			case 10:
				...
				break;
			case 11:
				act("$n utters the words 'I'll take that, gov'nor'",1,ch,0,0,TO_ROOM);
				cast_siphon_strength(GetMaxLevel(ch), ch, "", SPELL_TYPE_SPELL, vict, 0);
				break;
			case 12:
			case 13:
			...
			case 23:
			case 24:
				if(!affected_by_spell(vict,SPELL_DECAY) && !IS_SET(vict->M_immune, IMM_POISON)) {
			...
			case 31:
				if (!IS_SET(vict->M_immune, IMM_DRAIN)) {
					act("$n utters the words 'touched by the lord'",1,ch,0,0,TO_ROOM);
					cast_finger_of_death(GetMaxLevel(ch),ch,"",SPELL_TYPE_SPELL,vict,0);
				} else {
					act("$n utters the words 'bliss'",1,ch,0,0,TO_ROOM);
					cast_soul_steal(GetMaxLevel(ch),ch,"",SPELL_TYPE_SPELL,vict,0);
				}
			...
			default:
				if (!IS_SET(vict->M_immune, IMM_POISON)) {
					if (!(ch->equipment[WEAR_EYES])) {
						...
						cast_scourge_warlock(GetMaxLevel(ch),ch,"",SPELL_TYPE_SPELL, vict, 0);
					} else if(obj_index[(ch->equipment[WEAR_EYES])->item_number].virtual == TONGUE_ITEM) {
						act("$n utters the words 'oh golly'",1,ch,0,0,TO_ROOM);
						cast_scourge_warlock(GetMaxLevel(ch),ch,"",SPELL_TYPE_SPELL, vict, 0);
					} else {
					...
					act("$n utters the words 'touched by the lord'",1,ch,0,0,TO_ROOM);
					cast_finger_of_death(GetMaxLevel(ch),ch,"",SPELL_TYPE_SPELL,vict,0);
				} else { /* can't touch them, better get outta here */
					while (!room) {
...
//	log("entered druid proc");
	...
	if ((GET_HIT(ch) < (GET_MAX_HIT(ch) / 4)) && (lspell > 7) && (!IS_SET(ch->specials.act, ACT_AGGRESSIVE))) {
		...
		if (!IS_SET(real_roomp(ch->in_room)->room_flags, INDOORS)
				&& real_roomp(ch->in_room)->sector_type != SECT_AIR) {
			if(!number(0,2)) { /* cool, we're outtie! */
				while (!room) {
					to_room = number(0, top_of_world);
					room = real_roomp(to_room);
...
//	log("not fighting");
		if (GET_HIT(ch) < GET_MAX_HIT(ch)-10) {
			...
			switch(number(0,num)) {
				...
				case 2:
					...
					break;
				case 3:
					act("$n utters the words 'The Myrrhal shine upon me!'", 1, ch,0,0,TO_ROOM);
		...
		if (!ch->desc) { /* is it a mob? */
			...
			if (!affected_by_spell(ch,SPELL_PROTECT_FROM_GOOD) && !IS_GOOD(ch)) {
				act("$n utters the words 'hide from white'",FALSE,ch,0,0,TO_ROOM);
				cast_protection_from_good(GetMaxLevel(ch),ch,"",SPELL_TYPE_SPELL,ch,0);
				return(TRUE);
			}
			...
			if (GetMaxLevel(ch)>24) {
				...
				if (!affected_by_spell(ch,SPELL_ANIMAL_SUM_1) && OUTSIDE(ch) && !ch->followers
					...
					if (affected_by_spell(ch,SPELL_ANIMAL_SUM_1) && ch->followers) {
						...
						act("",FALSE,ch,0,0,TO_ROOM);
						do_group(ch, "all",0);
						act("\n\r$n utters the words 'size of the behemoth lord'",FALSE,ch,0,0,TO_ROOM);
...
//	log("pester em");
		...
		switch(lspell) {
			...
			case 7:
				...
				break;
			case 8:
				act("$n utters the words 'wrath of the serpent'",1,ch,0,0,TO_ROOM);
				cast_poison(GetMaxLevel(ch), ch, "", SPELL_TYPE_SPELL, vict, 0);
				break;
			case 9:
			case 10:
	...
	/* do heal */
		...
		switch(lspell) {
			...
			case 28:
				...
				break;
			default:
				act("$n utters the words 'Praise the Lord of Balance, I'm good to go another round!'", 1, ch,0,0,TO_ROOM);
				cast_heal(GetMaxLevel(ch), ch, "", SPELL_TYPE_SPELL, ch, 0);
				break;
		}
		return(TRUE);
				...
				if(ch->specials.remortclass == MONK_LEVEL_IND + 1) {
					...
					while (mainmonkskills[x].level != -1) {
						if(is_abbrev(arg,mainmonkskills[x].name)) {  //!str_cmp(arg,n_skills[x])){
							if(mainmonkskills[x].level > GET_LEVEL(ch,MONK_LEVEL_IND)) {
								send_to_char("$c0013[$c0015The Monk Guildmaster$c0013] tells you"
										" 'You're not experienced enough to learn this skill.'",ch);
								return(TRUE);
							}
							if(ch->skills[mainmonkskills[x].skillnum].learned > 45) {
								//check if skill already practiced
								send_to_char("$c0013[$c0015The Monk Guildmaster$c0013] tells you"
											 " 'You must learn from experience and practice to get"
											 " any better at that skill.'\n\r",ch);
							...
							sprintf(buf,"You practice %s for a while.\n\r",mainmonkskills[x].name);
							send_to_char(buf,ch);
							ch->specials.spells_to_learn--;
							if(!IS_SET(ch->skills[mainmonkskills[x].skillnum].flags,SKILL_KNOWN)) {
								SET_BIT(ch->skills[mainmonkskills[x].skillnum].flags,SKILL_KNOWN);
								SET_BIT(ch->skills[mainmonkskills[x].skillnum].flags,SKILL_KNOWN_MONK);
							}
							percent=ch->skills[mainmonkskills[x].skillnum].learned+int_app[GET_INT(ch)].learn;
							ch->skills[mainmonkskills[x].skillnum].learned = MIN(95,percent);
							if(ch->skills[mainmonkskills[x].skillnum].learned >= 95)
								send_to_char("'You are now a master of this art.'\n\r",ch);
							return(TRUE);
						}
						x++;
...
int astral_portal(struct char_data *ch, int cmd, char *arg, struct char_data *mob, int type)
{
  ...
  if(*buf) {
	  if (is_abbrev(buf,"pool") || is_abbrev(buf,"color pool")) {
			...
			if(ch->specials.remortclass == BARBARIAN_LEVEL_IND + 1) {
				...
				while (mainbarbskills[x].level != -1) {
					if(is_abbrev(arg,mainbarbskills[x].name)) {  //!str_cmp(arg,n_skills[x])){
						if(mainbarbskills[x].level > GET_LEVEL(ch,BARBARIAN_LEVEL_IND)) {
							send_to_char("$c0013[$c0015The Barbarian Guildmaster$c0013] tells you"
									" 'You're not experienced enough to learn this skill.'",ch);
							return(TRUE);
						}
						if(ch->skills[mainbarbskills[x].skillnum].learned > 45) {
							//check if skill already practiced
							send_to_char("$c0013[$c0015The Barbarian Guildmaster$c0013] tells you"
										 " 'You must learn from experience and practice to get"
										 " any better at that skill.'\n\r",ch);
						...
						sprintf(buf,"You practice %s for a while.\n\r",mainbarbskills[x].name);
						send_to_char(buf,ch);
						ch->specials.spells_to_learn--;
						if(!IS_SET(ch->skills[mainbarbskills[x].skillnum].flags,SKILL_KNOWN)) {
							SET_BIT(ch->skills[mainbarbskills[x].skillnum].flags,SKILL_KNOWN);
							SET_BIT(ch->skills[mainbarbskills[x].skillnum].flags,SKILL_KNOWN_BARBARIAN);
						}
						percent=ch->skills[mainbarbskills[x].skillnum].learned+int_app[GET_INT(ch)].learn;
						ch->skills[mainbarbskills[x].skillnum].learned = MIN(95,percent);
						if(ch->skills[mainbarbskills[x].skillnum].learned >= 95)
							send_to_char("'You are now a master of this art.'\n\r",ch);
						return(TRUE);
					}
					x++;
			...
			if(ch->specials.remortclass == RANGER_LEVEL_IND + 1) {
				...
				while (mainrangerskills[x].level != -1) {
					if(is_abbrev(arg,mainrangerskills[x].name)) {  //!str_cmp(arg,n_skills[x])){
						if(mainrangerskills[x].level > GET_LEVEL(ch,RANGER_LEVEL_IND)) {
							send_to_char("$c0013[$c0015The Ranger Guildmaster$c0013] tells you"
									" 'You're not experienced enough to learn this skill.'",ch);
							return(TRUE);
						}
						if(ch->skills[mainrangerskills[x].skillnum].learned > 45) {
							//check if skill already practiced
							send_to_char("$c0013[$c0015The Ranger Guildmaster$c0013] tells you"
										 " 'You must learn from experience and practice to get"
										 " any better at that skill.'\n\r",ch);
						...
						sprintf(buf,"You practice %s for a while.\n\r",mainrangerskills[x].name);
						send_to_char(buf,ch);
						ch->specials.spells_to_learn--;
						if(!IS_SET(ch->skills[mainrangerskills[x].skillnum].flags,SKILL_KNOWN)) {
							SET_BIT(ch->skills[mainrangerskills[x].skillnum].flags,SKILL_KNOWN);
							SET_BIT(ch->skills[mainrangerskills[x].skillnum].flags,SKILL_KNOWN_RANGER);
						}
						percent=ch->skills[mainrangerskills[x].skillnum].learned+int_app[GET_INT(ch)].learn;
						ch->skills[mainrangerskills[x].skillnum].learned = MIN(95,percent);
						if(ch->skills[mainrangerskills[x].skillnum].learned >= 95)
							send_to_char("'You are now a master of this art.'\n\r",ch);
						return(TRUE);
					}
					x++;
			...
			if(ch->specials.remortclass == PSI_LEVEL_IND + 1) {
				...
				while (mainpsiskills[x].level != -1) {
					if(is_abbrev(arg,mainpsiskills[x].name)) {  //!str_cmp(arg,n_skills[x])){
						if(mainpsiskills[x].level > GET_LEVEL(ch,PSI_LEVEL_IND)) {
							send_to_char("$c0013[$c0015The Psi Guildmaster$c0013] tells you"
									" 'You're not experienced enough to learn this skill.'",ch);
							return(TRUE);
						}
						if(ch->skills[mainpsiskills[x].skillnum].learned > 45) {
							//check if skill already practiced
							send_to_char("$c0013[$c0015The Psi Guildmaster$c0013] tells you"
										 " 'You must learn from experience and practice to get"
										 " any better at that skill.'\n\r",ch);
						...
						sprintf(buf,"You practice %s for a while.\n\r",mainpsiskills[x].name);
						send_to_char(buf,ch);
						ch->specials.spells_to_learn--;
						if(!IS_SET(ch->skills[mainpsiskills[x].skillnum].flags,SKILL_KNOWN)) {
							SET_BIT(ch->skills[mainpsiskills[x].skillnum].flags,SKILL_KNOWN);
							SET_BIT(ch->skills[mainpsiskills[x].skillnum].flags,SKILL_KNOWN_PSI);
						}
						percent=ch->skills[mainpsiskills[x].skillnum].learned+int_app[GET_INT(ch)].learn;
						ch->skills[mainpsiskills[x].skillnum].learned = MIN(95,percent);
						if(ch->skills[mainpsiskills[x].skillnum].learned >= 95)
							send_to_char("'You are now a master of this art.'\n\r",ch);
						return(TRUE);
					}
					x++;
			...
			if(ch->specials.remortclass == PALADIN_LEVEL_IND + 1) {
				...
				while (mainpaladinskills[x].level != -1) {
					if(is_abbrev(arg,mainpaladinskills[x].name)) {  //!str_cmp(arg,n_skills[x])){
						if(mainpaladinskills[x].level > GET_LEVEL(ch,PALADIN_LEVEL_IND)) {
							send_to_char("$c0013[$c0015The Paladin Guildmaster$c0013] tells you"
									" 'You're not experienced enough to learn this skill.'",ch);
							return(TRUE);
						}
						if(ch->skills[mainpaladinskills[x].skillnum].learned > 45) {
							//check if skill already practiced
							send_to_char("$c0013[$c0015The Paladin Guildmaster$c0013] tells you"
										 " 'You must learn from experience and practice to get"
										 " any better at that skill.'\n\r",ch);
						...
						sprintf(buf,"You practice %s for a while.\n\r",mainpaladinskills[x].name);
						send_to_char(buf,ch);
						ch->specials.spells_to_learn--;
						if(!IS_SET(ch->skills[mainpaladinskills[x].skillnum].flags,SKILL_KNOWN)) {
							SET_BIT(ch->skills[mainpaladinskills[x].skillnum].flags,SKILL_KNOWN);
							SET_BIT(ch->skills[mainpaladinskills[x].skillnum].flags,SKILL_KNOWN_PALADIN);
						}
						percent=ch->skills[mainpaladinskills[x].skillnum].learned+int_app[GET_INT(ch)].learn;
						ch->skills[mainpaladinskills[x].skillnum].learned = MIN(95,percent);
						if(ch->skills[mainpaladinskills[x].skillnum].learned >= 95)
							send_to_char("'You are now a master of this art.'\n\r",ch);
						return(TRUE);
					}
					x++;
