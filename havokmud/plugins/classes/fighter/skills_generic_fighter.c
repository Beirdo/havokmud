/*
 * HavokMUD - generic fighter skills
 * REQUIRED BY - ranger, barbarian, warrior, paladin, monk
 */
...
void do_tan(struct char_data *ch, char *arg, int cmd)
{
    ...
    if (!ch->skills[SKILL_TAN].learned) {
        send_to_char("Pardon?\n\r", ch);
        return;
    }
    if (MOUNTED(ch)) {
    ...
    if (j->affected[0].modifier != 0 && j->affected[1].modifier != 0) {
        ...
        if (percent > ch->skills[SKILL_TAN].learned) {
            /* 
             * make obj unusable for another tan
             */
            ...
            sprintf(buf, "%s tries to tan %s, but destroys it.",
    ...
    case RACE_DRAGON_TURTLE:
        ...
        sprintf(buf, "%s tans %s's hide.", GET_NAME(ch),
                j->short_description);
        act(buf, TRUE, ch, 0, 0, TO_ROOM);
        WAIT_STATE(ch, PULSE_VIOLENCE * 1);
        return;
    } else {
        send_to_char("Sorry, nothing left to make an item with.\n\r", ch);
        return;
    }
}
