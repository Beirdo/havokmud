         ...
         int type)
{
    ...
    for (v = rp->people; (v && (!found)); v = next) {
        next = v->next_in_room;
        /** @todo change that 100 to a #define */
        if (IS_NPC(v) && v->nr == 100 && CAN_SEE(ch, v)) {
            /*
             * is a zombie
             */
                ...
                struct char_data *mob, int type)
{
    ...
    for (i = roomFindNum(ch->in_room)->people; i; i = i->next_in_room) {
        if (IS_NPC(i) && i->nr == 6005 && !number(0, 3)) {
             ...
             struct char_data *mob, int type)
{
    ...
    for (i = rp->people; i; i = i->next_in_room) {
        if (IS_NPC(i) && i->nr == 6001 && !number(0, 3)) {
               ...
               struct char_data *mob, int type)
{
    ...
    if (type == EVENT_DEATH) {
        ...
        if ((tmp_ch = get_char_vis_world(ch, buf, 0))) {
            target_obj = objectRead(NADIA_KEY);
            objectGiveToChar(target_obj, tmp_ch);

            target_obj = objectRead(NADIA_PILL);
            objectGiveToChar(target_obj, tmp_ch);
        }
    }
               ...
               struct char_data *mob, int type)
{
    ...
    if (cmd == 56) {
        ...
        while (questitems[x][0] != -1) {
             ...
             */
            if ((obj = objectRead(questitems[x][0])) &&
                 ...
                 struct char_data *mob, int type)
{
    ...
    if (cmd == 72) {
        ...
        if (obj->item_number != universal) {
            ...
            if ((obj2 = objectRead(universal + 1))) {
                   ...
                   struct char_data *mob, int type)
{
    ...
    if (type == EVENT_DEATH) {
        ...
        case 0:
             ...
             */
            target_obj = objectRead(STRAHD2_ITEM1);
            objectGiveToChar(target_obj, mobtmp);

            target_obj = objectRead(STRAHD2_ITEM2);
            objectGiveToChar(target_obj, mobtmp);
              ...
              struct char_data *mob, int type)
{
    ...
    if (cmd == 531) {
        ...
        if (vict->nr == TMK_GUARD_ONE || vict->nr == TMK_GUARD_TWO) {
            switch (number(1, 10)) {
            case 1:
         ...
         int type)
{
    ...
    if (cmd == 531) {
        ...
        if (test == 0) {
            ...
            obj = objectRead(BLACK_PILL);
            objectGiveToChar(obj, ch);

            obj = objectRead(BLUE_PILL);
            objectGiveToChar(obj, ch);
                    ...
                    struct room_data *rp, int type)
{
    ...
    if ((i = gnome->carrying)) {
        ...
        if (winner) {
            ...
            if ((reward = objectRead(REWARD_GNOME))) {
...
int portal_regulator(struct char_data *ch, struct room_data *rp, int cmd)
{
    ...
    if (time_info.hours < 20 && time_info.hours > 5) {
        ...
        if (rp) {
            ...
            if (!check && (obj = objectRead(TARANTIS_PORTAL))) {
        ...
        if (rp) {
            ...
            if (!check && (obj = objectRead(REAVER_PORTAL))) {
                send_to_room("$c0008A sliver of darkness suddenly appears. It "
                             "widens, turns sideways, and becomes a "
                             "portal!\n\r", DEST_ROOM);
                 ...
                 struct char_data *mob, int type)
{
    ...
    if (cmd == 56) {
        ...
        if (obj1 && obj2 && obj3 && obj4 && obj5) {
            ...
            if ((obj = objectRead(SMITH_SHIELD))) {
                objectGiveToChar(obj, ch);
