...
void do_auth(struct char_data *ch, char *argument, int cmd)
{
    ...
    if (IS_IMMORTAL(d->character)) {    
        // dont let imms auth other imms since there is no check to see
        // if they can see the imm -gordon jan222003-
        send_to_char("That player was not found.\n\r", ch);
        return;
    }
    if (*argument) {
...
void do_bamfin(struct char_data *ch, char *arg, int cmd)
{
    ...
    if (!*arg) {
        ...
        /* show them their current bamf -Gordon jan202004- */
...
void do_bamfout(struct char_data *ch, char *arg, int cmd)
{
    ...
    if (!*arg) {
        ...
        /* again show them current bamf -Gordon jan202004- */
...
void do_emote(struct char_data *ch, char *arg, int cmd)
{
    ...
    /*
     * #if 1 i = 0; j = 0; k = 0; found = 0; sprintf(name, "");
     * sprintf(part1, ""); sprintf(oriarg, "%s",arg); if(!*arg) {
     * send_to_char("Yes, but what?\n\r",ch); return; } while(*arg &&
     * arg[0] != '*') { half_chop(arg, buf, arg); sprintf(part1, "%s
     * %s",part1, buf); } if(*arg && arg[0] == '*' && arg[1] == ' ') {
     * half_chop(arg, sign, arg); half_chop(arg, name, part2); found = 1;
     * }
     * 
     * if(found) { if (!(vict = get_char_room_vis(ch, name))) {
     * sprintf(buf, "Noone here by the name of %s.\n\r",name);
     * send_to_char(buf, ch); return; } else { sprintf(buf, "$n%s $N %s",
     * part1, part2); act(buf, FALSE, ch, 0, vict, TO_NOTVICT); act(buf,
     * FALSE, ch, 0, vict, TO_CHAR); sprintf(buf, "$n%s you %s", part1,
     * part2); act(buf, FALSE, ch, 0, vict, TO_VICT); } } else { for (i =
     * 0; *(oriarg + i) == ' '; i++);
     * 
     * if (!*(oriarg + i)) send_to_char("Yes.. But what?\n\r", ch); else {
     * sprintf(buf,"$n %s", oriarg + i); act(buf,FALSE,ch,0,0,TO_ROOM); if
     * (IS_SET(ch->specials.act,PLR_ECHO)) act(buf,FALSE,ch,0,0,TO_CHAR);
     * else send_to_char("Ok.\n\r",ch); } } #else log("enter"); i = 0; j =
     * 0; k = 0; found = 0; extra = 0; // sprintf(name, ""); // (remember
     * to make a *name) sprintf(part1, ""); sprintf(oriarg, "%s",arg);
     * if(!*arg) { log("no arg"); send_to_char("Yes, but what?\n\r",ch);
     * return; } while(*arg && arg[0] != '*'&& arg[1] != ' ') {
     * log("processing words before asterisk"); half_chop(arg, buf, arg);
     * sprintf(part1, "%s %s",part1, buf); } if(*arg && arg[0] == '*' &&
     * arg[1] != ' ') { sprintf(copy, "%s",arg);
     * 
     * for (; *copy == '*'; copy++);
     * 
     * for (; *copy; copy++, name++) { if(*copy == ' ') break; if(*copy ==
     * '*') { extra = 1; break; } *name = *copy; } *name = '\0'; if(extra)
     * for (; !isspace(*copy); copy++); // get rid of any junk til we meet
     * a space
     * 
     ...
     * 
     * found = 1; log("found legal asterisk"); }
     * 
     * if(found) { log("entered the complicated emote"); // see if there's
     * a dude in the room with this name if (!(vict = get_char_room_vis(ch, 
     * name))) { log("no target found"); sprintf(buf, "Noone here by the
     * name of %s.\n\r",name); send_to_char(buf, ch); return; } else {
     * log("target found"); // now fix up buffers, do the acts if(extra)
     * sprintf(buf, "$n%s $N's %s", part1, part2); else sprintf(buf, "$n%s
     * $N %s", part1, part2); act(buf, FALSE, ch, 0, vict, TO_NOTVICT);
     * act(buf, FALSE, ch, 0, vict, TO_CHAR); if(extra) sprintf(buf, "$n%s
     * your %s", part1, part2); else sprintf(buf, "$n%s you %s", part1,
     * part2); act(buf, FALSE, ch, 0, vict, TO_VICT); } } else {
     * log("entered the simple emote"); for (i = 0; *(oriarg + i) == ' ';
     * i++);
     * 
     * if (!*(oriarg + i)) send_to_char("Yes.. But what?\n\r", ch); else {
     * sprintf(buf,"$n %s", oriarg + i); act(buf,FALSE,ch,0,0,TO_ROOM); if
...
void roll_abilities(struct char_data *ch)
{
    ...
    // Modified this to give more chances to players...
    ...
    if (HasClass(ch, CLASS_WARRIOR | CLASS_BARBARIAN | 
        ...
        if (ch->abilities.str > 18 && 
            ...
            /* was a half-giant so * just make 100 */
...
void do_start(struct char_data *ch)
{
    ...
    // roll_abilities(ch); This is done earlier in the creation process
    // now...
    /*
...
void do_disconnect(struct char_data *ch, char *argument, int cmd)
{
    ...
    /*
     * if(!(victim = get_char(arg))) { send_to_char("No-one by that name
     * in the world.\n\r",ch); return; }
     * 
     * if ( victim->desc == NULL ) { act("$N doesn't have a descriptor.",
     * 0, ch, 0, victim, TO_CHAR); return; } 
     */
...
void do_home(struct char_data *ch, char *argument, int cmd)
{
    ...
    if (ch->player.hometown > 0) {
        ...
        // char_from_room(ch);
        // char_to_room(ch,ch->specials.start_room);
        // act("A whirling vortex appears and $n
        // arrives.",FALSE,ch,0,0,TO_ROOM);
        do_look(ch, "", 15);
    } else {
...
void do_wizset(struct char_data *ch, char *argument, int cmd)
{
    ...
    if (!strcmp("fast", flag)) {
        ...
        /*
         * if (!*name) { send_to_char("Set to what room?\n\r",ch); } else
         * { if (atoi(name) >0 && atoi(name) < WORLD_SIZE)
         * ch->player.hometown=atoi(name); else send_to_char("Invalid room 
         * number.\n\r",ch); } lemme do it my own way :P 
         */
...
void do_setobjmax(struct char_data *ch, char *argument, int cmd)
{
    ...
    send_to_char(buf, ch);
    /*
     * obj = read_object(rnum, REAL); if (obj) {
     * obj_index[rnum].MaxObjCount = number; sprintf(buf,"Maximum object
     * count set to %d on object %s. \n\r",number,(char *) obj->name);
     * send_to_char(buf,ch); / Temporary.. Object doesnt' get destroyed
     * here / obj_index[obj->item_number].number--; } else
...
void do_startarena(struct char_data *ch, char *argument, int cmd)
{
    ...
    if ((tmp1 == 0 && tmp2 == 0)) {
        ...
        // if (ArenaNoBash == 1) {
        // sprintf(buf, "$c000cGrouping in the arena is not
        // allowed.\n\r");
        // send_to_all(buf);
        // }
