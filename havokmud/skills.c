...
case RACE_WILD_ELF  :
case RACE_SEA_ELF   :
case RACE_MOON_ELF  :sprintf(hidetype,"elf hide");
			lev=(int)lev/2;
			break;
case RACE_AVARIEL   :sprintf(hidetype,"feathery elf hide");
			lev=(int)lev/2;
			break;
...
case RACE_FOREST_GNOME:
case RACE_ROCK_GNOME :sprintf(hidetype,"gnome hide");
			lev=(int)lev/2;
			break;
...
dlog("in do_stop");
	...
	if(ch->specials.is_playing) {
		...
		act("$n lets the notes fade from $s voice as $e stops playing.",FALSE, ch, 0, 0, TO_ROOM);
		...
		for (tmp = rp->people; tmp; tmp = tmp2) {
			...
			if(tmp->specials.is_playing == spl)
				return;
		}
		for (tmp = rp->people;tmp;tmp=tmp2) {
			tmp2 = tmp->next_in_room;
...
dlog("in do_throw_voice");
	...
	if (ch->skills[SKILL_THROW_VOICE].learned) {
		if (percent > ch->skills[SKILL_THROW_VOICE].learned) {
			...
			MissileDamage(ch, vict, dam, SKILL_THROW_VOICE);
