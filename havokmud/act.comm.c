...
void do_say(struct char_data *ch, char *argument, int cmd)
{
    ...
    if (!argument) {
        ...
        if (IS_NPC(ch) || (IS_SET(ch->specials.act, PLR_ECHO))) {
            oldSendOutput(ch, "$c0015You say '%s'\n\r", argument);
        }
    }
}
...
void do_commune(struct char_data *ch, char *argument, int cmd)
{
    ...
    if (IS_NPC(ch) || IS_SET(ch->specials.act, PLR_ECHO)) {
        oldSendOutput(ch, "$c0012You think '%s'", argument);
    }
...
void do_tell(struct char_data *ch, char *argument, int cmd)
{
    ...
    oldSendOutput(vict, "$c0013[$c0015%s$c0013] tells you '%s'",
    ...
    if (IS_NPC(ch) || IS_SET(ch->specials.act, PLR_ECHO)) {
        oldSendOutput(ch, "$c0013You tell %s %s'%s'",
...
void do_write(struct char_data *ch, char *argument, int cmd)
{
     ...
     */
    if (pen->type_flag != ITEM_TYPE_PEN) {
        act("$p is no good for writing with.", FALSE, ch, pen, 0, TO_CHAR);
    } else if (paper->type_flag != ITEM_TYPE_NOTE) {
...
void do_speak(struct char_data *ch, char *argument, int cmd)
{
    ...
    oldSendOutput(ch, "You concentrate on speaking %s.\n\r", 
...
void do_split(struct char_data *ch, char *argument, int cmd)
{
    ...
    oldSendOutput(ch, "You split %d gold coins.  Your share is %d gold coins.\n\r",
                  amount, share + extra);
...
void do_ooc(struct char_data *ch, char *argument, int cmd)
{
    ...
    if (IS_NPC(ch) || IS_SET(ch->specials.act, PLR_ECHO)) {
        oldSendOutput(ch, "$c0012You $c0015OOC$c0012 '$c0015%s$c0012'", argument);
    }
...
void do_reply(struct char_data *ch, char *argument, int cmd)
{
    ...
    oldSendOutput(vict, "$c0013[$c0015%s$c0013] replies to you '%s'",
	...
	*/
    if (IS_NPC(ch) || IS_SET(ch->specials.act, PLR_ECHO)) {
        oldSendOutput(ch, "$c0013You reply to %s %s'%s'",
                  (IS_NPC(vict) ? vict->player.short_descr : GET_NAME(vict)),
                  (IS_AFFECTED2(vict, AFF2_AFK) ? "(who is AFK) " : ""),
                  argument);
    }
