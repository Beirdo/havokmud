	  ...
	  !check_soundproof(i->character)) {
  	 ...
  	 if (i->character->in_room != NOWHERE)  {
		...
		&&GetMaxLevel(ch) < LOW_IMMORTAL ) 	{
		 act(buf1, 0, ch, 0, i->character, TO_VICT);
	    } else if (GetMaxLevel(ch) >= LOW_IMMORTAL) /* My addons from here... - Manwe */

                  {
                     sprintf(buf3, "$c0011[$c0015$n$c0011] yells across the world '%s'", argument);
...
if (!ch->skills) {
	...
	switch(ch->player.speaks) {
		...
		case SPEAK_GNOMISH:learned = ch->skills[LANG_GNOMISH].learned;
				...
				break;
                case SPEAK_ALL:learned = 100;             //Speak all procedure
                                skill_num=LANG_COMMON;    //continues here part2
	                        break;                    // -Manwe Windmaster 200697
...
if (!(*argument))
  ...
  else
  {
   ...
   if (ii>number(1,101)) {
    ...
    sprintf(buf1, "$c0014:*:$c0012$n$c0014:*: prays '$c0012%s$c0014'", argument);
...
/* OOC Command, for roleplaying...  -Manwe Windmaster */
...
dlog("in do_reply");
  ...
  if (check_soundproof(vict)) {
	...
	return;
	 ...
	 send_to_char("That person is not near enough for you to reply, bad :(.\n\r",ch);
 	 return;
	}
#endif
  ...
  if (IS_NPC(ch) || IS_SET(ch->specials.act, PLR_ECHO)) {
     sprintf(buf,"$c0013You reply to %s %s'%s'",
	  (IS_NPC(vict) ? vict->player.short_descr : GET_NAME(vict)), \
          (IS_AFFECTED2(vict, AFF2_AFK) ? "(who is AFK) " : ""), argument);
     act(buf,FALSE, ch,0,0,TO_CHAR);
  }
	...
	if(!*name)
		...
		if(IS_NPC(vict)) {
			if(strcmp(vict->specials.talks,"")) {
				...
				act("You try to strike up a conversation with $N, but $E has nothing to tell you.",FALSE,ch,0,vict,TO_CHAR);
			}
		} else
