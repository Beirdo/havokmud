 ...
 *
 * Revision 1.1.1.1  2001/02/12 22:58:09  manwewindmaster
 * Initial import into CVS. This is production release 1.7 with a preliminary
 * bugfix for the death crash bug
 *
...
static          putfreehdr();
static Freehdr  freehdrptr = NIL;       /* ptr to block of available
                                         * headers */
 ...
 * error checking 
 */
static          error();        /* sets errno; prints msg and aborts if
                                 * DEBUG is on */
...
static char    *shrink(oldblk, oldsize, newsize)
 ...
 *                                              if 'flag' == 1).
 */
static int reclaim(oldblk, oldsize, flag)
     register Dblk oldblk;
     uint oldsize;
     int flag;
{
        ...
        if (oldblk == freeblk) {
            ...
            if (oldblk >= freeneighbor) {
                ...
                if (oldneighbor > freeneighbor) {
                     ...
                     */
                    freeblk->size -=
                        ((char *) freeneighbor - (char *) oldblk);
                    /*
                     * split the right fragment by
                     * setting oldblk's neighbor's size
                     */
                    oldneighbor->size =
                        (char *) freeneighbor - (char *) oldneighbor;
                    /*
     ...
     uint nbytes;
{
    ...
    if (nbpg == 0) {
        nbpg = getpagesize();
        /*
         * hack to avoid fragmenting the heap with the first freehdr page 
         */
        if ((newhdr = getfreehdr()) == NIL) {
            /*
             * Error message returned by getfreehdr() 
...
int malloc_verify()
{
    ...
    while (p < (Dblk) _ubound) {
        size = p->size;
        if ((size) < SMALLEST_BLK || (size) & (ALIGNSIZ - 1)
            || (size) > heapsize()
            || ((char *) (p)) + (size) > _ubound) {
 ...
 * VARARGS2
 */
static dummy_sprintf(string, fmt, x1, x2, x3)
     char           *string;
     register char  *fmt;
     uint x1,
         x2,
 ...
 * VARARGS1
 */
static error(fmt, arg1, arg2, arg3)
     char           *fmt;
     int arg1,
         arg2,
         arg3;
{
    static          n = 0;      /* prevents infinite recursion when using
                                 * stdio */
    register int    nbytes;
     ...
     Freehdr p;
{
    ...
    if (p != NIL) {
        ...
        printf("(%#lx): (left = %#lx, right = %#lx, len = %d)\n",
               ...
               (p->right ? p->right->block : 0), p->size);
        prtree(p->left);
    }
    depth--;
}
