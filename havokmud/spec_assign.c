...
struct special_proc_entry mobSpecials[] = {
    {1, remort_guild},
    {2, Ringwraith},
    {3, tormentor},
#if 0
...
struct special_proc_entry objSpecials[] = {
    {15, SlotMachine},
    {30, scraps},
    {23, ships_helm},   
    {31, portal},
...
int objSpecialCount = NELEMS(objSpecials);

struct special_proc_entry roomSpecials[] = {
    {99, Donation},
    {13510, Donation},
...
void assign_mobiles( void )
{
    ...
    struct special_proc_entry *mob;

    for (i = 0; i < mobSpecialCount; i++) {
        mob = &mobSpecials[i];

        if( !procIsRegistered( mob->func, PROC_MOBILE ) ) {
            sprintf( buf, "assign_mobiles: proc for mobile %d not registered.",
                     mob->vnum );
            Log( buf );
            continue;
        }

        rnum = real_mobile(mob->vnum);
        if (rnum < 0) {
...
void assign_objects( void )
{
    int             i,
                    rnum;
    char            buf[MAX_STRING_LENGTH];
    struct special_proc_entry *obj;

    for (i = 0; i < objSpecialCount; i++) {
        obj = &objSpecials[i];

        if( !procIsRegistered( obj->func, PROC_OBJECT ) ) {
            sprintf( buf, "assign_objects: proc for object %d not registered.",
                     obj->vnum );
            Log( buf );
            continue;
        }

        rnum = real_object(obj->vnum);
        if (rnum < 0) {
...
void assign_rooms( void )
{
    ...
    struct room_data *rp;
    char            buf[MAX_STRING_LENGTH];
    struct special_proc_entry *room;

    for (i = 0; i < roomSpecialCount; i++) {
        room = &roomSpecials[i];

        if( !procIsRegistered( room->func, PROC_ROOM ) ) {
            sprintf( buf, "assign_rooms: proc for room %d not registered.",
                     room->vnum );
            Log( buf );
            continue;
        }

        rp = real_roomp(room->vnum);
        if (!rp) {
            sprintf(buf, "assign_rooms: Room %d not found in database.",
                    room->vnum);
            Log(buf);
            continue;
        }
