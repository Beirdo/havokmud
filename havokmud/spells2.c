		...
		struct char_data *tar_ch, struct obj_data *tar_obj )
{
  ...
  case SPELL_TYPE_SPELL:
    ...
    if (ch != tar_ch)
      act("$N is protected by the spirits of god.", FALSE, ch, 0, tar_ch, TO_CHAR);
...
-Aura of Power
*/
void cast_aura_power( byte level, struct char_data *ch, char *arg,
     int type, struct char_data *tar_ch, struct obj_data *tar_obj )
{
  switch(type) {
  case SPELL_TYPE_SPELL:
...
void cast_enlightenment( byte level, struct char_data *ch, char *arg,
     int type, struct char_data *tar_ch, struct obj_data *tar_obj )
{
  switch(type) {
  case SPELL_TYPE_SPELL:
...
void play_song_of_opening( byte level, struct char_data *ch, char *arg, int type, struct char_data *tar_ch, struct obj_data *tar_obj )
{
	int door;
	char dir[MAX_INPUT_LENGTH], buf[MAX_INPUT_LENGTH];
	char otype[MAX_INPUT_LENGTH];
	...
	switch(type) {
		case SPELL_TYPE_SPELL:
		{
			...
			if (generic_find(arg, FIND_OBJ_INV | FIND_OBJ_ROOM, ch, &victim, &obj)) {
				send_to_char("Sorry - this song only opens door locks.\n", ch);
				return;
			} else if ((door = find_door(ch, otype, dir)) >= 0) {
				if (!IS_SET(EXIT(ch, door)->exit_info, EX_ISDOOR))
					send_to_char("That's absurd.\n\r", ch);
					...
					send_to_char("You can't seem to spot any lock to charm.\n\r", ch);
				else if (!IS_SET(EXIT(ch, door)->exit_info, EX_LOCKED))
					send_to_char("Oh.. it wasn't locked at all.\n\r", ch);
				else if (IS_SET(EXIT(ch, door)->exit_info, EX_PICKPROOF))
					send_to_char("You seem to be unable to charm this lock.\n\r", ch);
				else {
					if ((EXIT(ch, door)->keyword) && strcmp("secret", fname(EXIT(ch, door)->keyword)))
						act("$n hums a little tune, and the $F unlocks.", 0, ch, 0, EXIT(ch, door)->keyword, TO_ROOM);
					else
						act("$n hums a little tune, and the lock clicks open.", TRUE, ch, 0, 0, TO_ROOM);
						send_to_char("The lock quickly yields to your charms.\n\r", ch);
						raw_unlock_door(ch, EXIT(ch, door), door);
				}
			}
		}
		...
		default:
			log("serious error in play_song_of_opening.");
			break;
	}
}

void play_song_of_comprehension( byte level, struct char_data *ch, char *arg, int type, struct char_data *tar_ch, struct obj_data *tar_obj )
{
		if(!HasInstrument(ch))
			return;
	...
	switch(type) {
		case SPELL_TYPE_SPELL:
			song_of_comprehension(level, ch, tar_ch, 0);
			break;
		default:
			log("serious screw-up in play_song_of_comprehension");
			break;
	}
}

void play_song_of_biting_words( byte level, struct char_data *ch, char *arg, int type, struct char_data *tar_ch, struct obj_data *tar_obj )
{
	switch(type) {
		case SPELL_TYPE_SPELL:
...
void play_song_of_the_lullaby( byte level, struct char_data *ch, char *arg, int type, struct char_data *tar_ch, struct obj_data *tar_obj )
{
	if(!HasInstrument(ch))
		return;
	switch(type) {
		case SPELL_TYPE_SPELL:
			...
			break;
		default:
			log("serious screw-up in play_song_of_the_lullaby");
			break;
	}
}

void play_song_of_charming( byte level, struct char_data *ch, char *arg, int type, struct char_data *tar_ch, struct obj_data *tar_obj )
{
	if(!HasInstrument(ch))
		return;
	switch(type) {
		case SPELL_TYPE_SPELL:
			song_of_charming(level, ch, tar_ch, 0);
			break;
		default:
			log("serious screw-up in play_song_of_charming");
			break;
	}
}

void play_song_of_freezing_north( byte level, struct char_data *ch, char *arg, int type, struct char_data *tar_ch, struct obj_data *tar_obj )
{
	if(!HasInstrument(ch))
		return;
	switch(type) {
		case SPELL_TYPE_SPELL:
			spell_chill_touch(level, ch, tar_ch, 0);
			break;
		default:
			log("serious screw-up in play_song_of_freezing_north");
			break;
	}
}

void play_song_of_enthrallment( byte level, struct char_data *ch, char *arg, int type, struct char_data *tar_ch, struct obj_data *tar_obj )
{
	if(!HasInstrument(ch))
		return;
	switch(type) {
		case SPELL_TYPE_SPELL:
			song_of_enthrallment(level, ch, tar_ch, 0);
			break;
		default:
			log("serious screw-up in play_song_of_enthrallment");
			break;
	}
}

void play_song_of_calming( byte level, struct char_data *ch, char *arg, int type, struct char_data *tar_ch, struct obj_data *tar_obj )
{
	if(!HasInstrument(ch))
		return;
	switch(type) {
		case SPELL_TYPE_SPELL:
			song_of_calming(level, ch, 0, 0);
			break;
		default:
			log("serious screw-up in play_song_of_calming");
			break;
	}
}

void play_song_of_the_guardian( byte level, struct char_data *ch, char *arg, int type, struct char_data *tar_ch, struct obj_data *tar_obj )
{
	if(!HasInstrument(ch))
		return;
	switch(type) {
		case SPELL_TYPE_SPELL:
			song_of_the_guardian(level, ch, 0, 0);
			break;
		default:
			log("serious screw-up in play_song_of_the_guardian");
			break;
	}
}

void play_song_of_muscle( byte level, struct char_data *ch, char *arg, int type, struct char_data *tar_ch, struct obj_data *tar_obj )
{
	if(!HasInstrument(ch))
		return;
	switch(type) {
		case SPELL_TYPE_SPELL:
			song_of_muscle(level, ch, tar_ch, 0);
			break;
		default:
			log("serious screw-up in play_song_of_muscle");
			break;
	}
}
...
void play_song_of_seeing( byte level, struct char_data *ch, char *arg, int type, struct char_data *tar_ch, struct obj_data *tar_obj )
{
	if(!HasInstrument(ch))
		return;
	switch(type) {
...
void play_song_of_spell_breaching( byte level, struct char_data *ch, char *arg, int type, struct char_data *tar_ch, struct obj_data *tar_obj )
{
	if(!HasInstrument(ch))
		return;
	switch(type) {
		case SPELL_TYPE_SPELL:
			...
			break;
		default:
			log("serious screw-up in play_song_of_spell_breaching");
			break;
	}
}

void play_song_of_lightning( byte level, struct char_data *ch, char *arg, int type, struct char_data *tar_ch, struct obj_data *tar_obj )
{
	if(!HasInstrument(ch))
		return;
	switch(type) {
		case SPELL_TYPE_SPELL:
			spell_lightning_bolt(level, ch, tar_ch, 0);
			break;
		default:
			log("serious screw-up in play_song_of_lightning");
			break;
	}
}

void play_song_of_levitation( byte level, struct char_data *ch, char *arg, int type, struct char_data *tar_ch, struct obj_data *tar_obj )
{
	if(!HasInstrument(ch))
		return;
	switch(type) {
		case SPELL_TYPE_SPELL:
			song_of_levitation(level, ch, tar_ch, 0);
			break;
		default:
			log("serious screw-up in play_song_of_levitation");
			break;
	}
}

void play_song_of_the_winter_gale( byte level, struct char_data *ch, char *arg, int type, struct char_data *tar_ch, struct obj_data *tar_obj )
{
	if(!HasInstrument(ch))
		return;
	switch(type) {
		case SPELL_TYPE_SPELL:
			...
			break;
		default:
			log("serious screw-up in play_song_of_the_winter_gale");
			break;
	}
}

void play_song_of_dazzling( byte level, struct char_data *ch, char *arg, int type, struct char_data *tar_ch, struct obj_data *tar_obj )
{
	if(!HasInstrument(ch))
		return;
	switch(type) {
...
void play_song_of_the_hunt( byte level, struct char_data *ch, char *arg, int type, struct char_data *tar_ch, struct obj_data *tar_obj )
{
	if(!HasInstrument(ch))
		return;
	switch(type) {
...
void play_song_of_heraldry( byte level, struct char_data *ch, char *arg, int type, struct char_data *tar_ch, struct obj_data *tar_obj )
{
	if(!HasInstrument(ch))
		return;
	switch(type) {
		case SPELL_TYPE_SPELL:
			...
			break;
		default:
			log("serious screw-up in play_song_of_heraldry");
			break;
	}
}

void play_song_of_legendary_weapon( byte level, struct char_data *ch, char *arg, int type, struct char_data *tar_ch, struct obj_data *tar_obj )
{
	if(!HasInstrument(ch))
		return;
	switch(type) {
		case SPELL_TYPE_SPELL:
			...
			break;
		default:
			log("serious screw-up in play_song_of_legendary_weapon");
			break;
	}
}

void play_song_of_lethargy( byte level, struct char_data *ch, char *arg, int type, struct char_data *tar_ch, struct obj_data *tar_obj )
{
	if(!HasInstrument(ch))
		return;
	switch(type) {
...
void play_song_of_spirits( byte level, struct char_data *ch, char *arg, int type, struct char_data *tar_ch, struct obj_data *tar_obj )
{
	if(!HasInstrument(ch))
		return;
	switch(type) {
		case SPELL_TYPE_SPELL:
			song_of_spirits(level, ch, tar_ch, 0);
			break;
		default:
			log("serious screw-up in play_song_of_spirits");
			break;
	}
}

void play_song_of_the_high_hunt( byte level, struct char_data *ch, char *arg, int type, struct char_data *tar_ch, struct obj_data *tar_obj )
{
	if(!HasInstrument(ch))
		return;
	switch(type) {
...
void play_song_of_silence( byte level, struct char_data *ch, char *arg, int type, struct char_data *tar_ch, struct obj_data *tar_obj )
{
	if(!HasInstrument(ch))
		return;
	switch(type) {
...
void play_song_of_speed( byte level, struct char_data *ch, char *arg, int type, struct char_data *tar_ch, struct obj_data *tar_obj )
{
	if(!HasInstrument(ch))
		return;
	switch(type) {
...
void play_song_of_summoner( byte level, struct char_data *ch, char *arg, int type, struct char_data *tar_ch, struct obj_data *tar_obj )
{
	if(!HasInstrument(ch))
		return;
	switch(type) {
		case SPELL_TYPE_SPELL:
			song_of_summoner(level, ch, tar_ch, 0);
			break;
		default:
			log("serious screw-up in play_song_of_summoner");
			break;
	}
}

void play_song_of_sight( byte level, struct char_data *ch, char *arg, int type, struct char_data *tar_ch, struct obj_data *tar_obj )
{
	if(!HasInstrument(ch))
		return;
	switch(type) {
		case SPELL_TYPE_SPELL:
			song_of_sight(level, ch, tar_ch, 0);
			break;
		default:
			log("serious screw-up in play_song_of_sight");
			break;
	}
}

void play_song_of_the_hearth( byte level, struct char_data *ch, char *arg, int type, struct char_data *tar_ch, struct obj_data *tar_obj )
{
	if(!HasInstrument(ch))
		return;
	switch(type) {
		case SPELL_TYPE_SPELL:
			song_of_the_hearth(level, ch, 0, 0);
			break;
		default:
			log("serious screw-up in play_song_of_the_hearth");
			break;
	}
}

void play_song_of_eternal_light( byte level, struct char_data *ch, char *arg, int type, struct char_data *tar_ch, struct obj_data *tar_obj )
{
	if(!HasInstrument(ch))
		return;
	switch(type) {
		case SPELL_TYPE_SPELL:
			song_of_eternal_light(level, ch, 0, 0);
			break;
		default:
			log("serious screw-up in play_song_of_eternal_light");
			break;
	}
}

void play_song_of_wanderer( byte level, struct char_data *ch, char *arg, int type, struct char_data *tar_ch, struct obj_data *tar_obj )
{
	if(!HasInstrument(ch))
		return;
	switch(type) {
		case SPELL_TYPE_SPELL:
			song_of_wanderer(level, ch, 0, 0);
			break;
		default:
			log("serious screw-up in play_song_of_wanderer");
			break;
	}
}

void play_sounds_of_fear( byte level, struct char_data *ch, char *arg, int type, struct char_data *tar_ch, struct obj_data *tar_obj )
{
	if(!HasInstrument(ch))
		return;
	switch(type) {
...
void play_heros_chant( byte level, struct char_data *ch, char *arg, int type, struct char_data *tar_ch, struct obj_data *tar_obj )
{
	if(!HasInstrument(ch))
		return;
	switch(type) {
