    ...
    if (!found) {
        if (Heap->str) {
            ...
            Heap->str = (struct StrHeapList *)
                        malloc(sizeof(struct StrHeapList));
        }
...
struct StrHeap *InitHeap()
{
    ...
    Heap->str = 0;
    /*
     * Heap->str = (struct StrHeapList *)malloc(sizeof(struct
     * StrHeapList)); Heap->str[0].string=0; Heap->str[0].total=0; 
     */
    Heap->uniq = 0;
    return (Heap);
}
    ...
    for (i = 0; i < Heap->uniq; i++) {
        if (type != TO_CHAR) {
            if (Heap->str[i].total > 1) {
                sprintf(buf, "%s [%d]", Heap->str[i].string,
                        Heap->str[i].total);
            } else {
