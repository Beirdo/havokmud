...
char *help_edit_menu = "    1) Name(s) of file         2) Usage\n\r"
...
void do_hedit(struct char_data *ch, char *argument, int cmd)
{
	...
	if(argument) {
		if(!(read_help_from_file(ch, argument, 2))) {
...
int is_help_elem(struct help_file_u *hlp, char *arg)
{
	/* checks both old and new help files for most common formats, and writes them into
	   their own place in the help_file_u format */
	int i, j;
...
/* ignore formats of this kind */
	...
	for(i=0; i<18; i++) {
		if(arg[i] != usage[i])
			break;
		if(i == 17) {/* first 18 chars are identical, should be good */
			half_chop(arg, arg1, arg2); /* put everything from : to end of line into arg2 */
			half_chop(arg2, arg1, arg3); /* get rid of the : */
...
void HelpEdit(struct char_data *ch, char *arg)
{
	if(ch->specials.hedit == HELP_MAIN_MENU) {
		...
		switch(atoi(arg)) {
           case 0: UpdateHelpMenu(ch);
                   return;
           case 1: ch->specials.hedit = CHANGE_HELP_NAME;
                   ChangeHelpName(ch, "", ENTER_CHECK);
                   return;
           case 2: ch->specials.hedit = CHANGE_HELP_USAGE;
                   ChangeHelpUsage(ch, "", ENTER_CHECK);
                   return;
           case 3: ch->specials.hedit = CHANGE_HELP_ACCUMULATIVE;
                   ChangeHelpAccumulative(ch, "", ENTER_CHECK);
                   return;
           case 4: ch->specials.hedit = CHANGE_HELP_DURATION;
                   ChangeHelpDuration(ch, "", ENTER_CHECK);
                   return;
           case 5: ch->specials.hedit = CHANGE_HELP_LEVEL;
                   ChangeHelpLevel(ch, "", ENTER_CHECK);
                   return;
           case 6: ch->specials.hedit = CHANGE_HELP_DAMTYPE;
                   ChangeHelpDamtype(ch, "", ENTER_CHECK);
                   return;
           case 7: ch->specials.hedit = CHANGE_HELP_SAVES;
                   ChangeHelpSaves(ch, "", ENTER_CHECK);
                   return;
           case 8: ch->specials.hedit = CHANGE_HELP_DESCRIPTION;
                   ChangeHelpDescription(ch, "", ENTER_CHECK);
                   return;
           case 9: ch->specials.hedit = CHANGE_HELP_REFERENCES;
                   ChangeHelpReferences(ch, "", ENTER_CHECK);
                   return;
           case 10: ch->specials.hedit = CHANGE_HELP_WIZARD;
                   ChangeHelpWizard(ch, "", ENTER_CHECK);
                   return;

			case 99:
				ch->desc->connected = CON_PLYNG;
				act("$n has returned from editing a helpfile.", FALSE, ch, 0, 0, TO_ROOM);
...
int write_help_to_file(struct char_data *ch, struct help_file_u *hlp)
{
	...
	if(!hlp) {
		send_to_char("Trying to save a non-existent help file (write_help_to_file). Aborting.\n\r", ch);
		return(FALSE);
	}
