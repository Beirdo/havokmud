...
void do_disengage(struct char_data *ch, int cmd)
{
int percent;
  ...
  if((GET_POS(ch) == POSITION_FIGHTING) && !ch->specials.fighting){
      ...
      act("$n looks around sheepishly and stops fighting $s shadow.",TRUE,ch, 0, 0, TO_ROOM);
      WAIT_STATE(ch, PULSE_VIOLENCE);
      return;
    }
  if(!ch->specials.fighting){
  ...
  if(!ch->skills[SKILL_DISENGAGE].learned)
    {send_to_char("You aren't really sure how disengage without hurting yourself.\n\r", ch);
     return;
    }
  ...
  if(percent > ch->skills[SKILL_DISENGAGE].learned)  
    ...
    act("$n tries to back out of combat with $N, but seems to stumble.",TRUE, ch, 0, ch->specials.fighting, TO_NOTVICT);
    act("$n tries to back out of combat with you, but seems to stumble.",TRUE, ch, 0, ch->specials.fighting, TO_VICT);
    LearnFromMistake(ch,SKILL_DISENGAGE,0,90);
    WAIT_STATE(ch,PULSE_VIOLENCE*2);
    return;
    }else{
    send_to_char("You step back and try to disengage\n\r",ch);
    act("$n backs out of combat with $N in an attempt to disengage",TRUE,ch, 0, ch->specials.fighting, TO_NOTVICT);
    act("$n tries to back out of combat with you in an attempt to disengage.",TRUE, ch, 0, ch->specials.fighting, TO_VICT);
    stop_fighting(ch);
    WAIT_STATE(ch,PULSE_VIOLENCE*2);
    return;
    }
