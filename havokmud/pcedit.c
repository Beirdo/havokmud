...
void muck(int orig_ammt, char name[80]) {
  ...
  printf("\nChecking :%d:%s\n", i, dummy[i]->grunt.name);
	...
	printf("Levels: M:%d C:%d W:%d T:%d D:%d B:%d S:%d P:%d R:%d P:%d ",
          ...
          dummy[i]->grunt.level[1],
          dummy[i]->grunt.level[2],
          dummy[i]->grunt.level[3],
          dummy[i]->grunt.level[4],
          dummy[i]->grunt.level[5],
          dummy[i]->grunt.level[6],
          dummy[i]->grunt.level[7],
          dummy[i]->grunt.level[8],
          dummy[i]->grunt.level[9],
          dummy[i]->grunt.level[10]);

    printf("\nCurrent STR is %d.",dummy[i]->grunt.abilities.str);
    printf("\nCurrent STR_add is %d.",dummy[i]->grunt.abilities.str_add);
    printf("\nCurrent INT is %d.",dummy[i]->grunt.abilities.intel);
	...
	break;
    case 4:
         printf("\n 0) exit 1) str 2) int 3) wis 4) dex 5) con 6) cha 7) str_add\n");
         printf("\nWhich stat?? -> ");
         m=get_int(0,7,TRUE);
         switch ( m ) {
           case 1:
	     ...
	     printf("\nSTR set to %d.",dummy[i]->grunt.abilities.str);
	     break;
           case 2:
	     printf("\nCurrent INT is %d.",dummy[i]->grunt.abilities.intel);
	     printf("\nNew INT? -> \n");
	     dummy[i]->grunt.abilities.intel=(get_int(1,25,FALSE));
	     printf("\nINT set to %d.",dummy[i]->grunt.abilities.intel);
	     break;
           case 3:
	     printf("\nCurrent WIS is %d.",dummy[i]->grunt.abilities.wis);
	     printf("\nNew WIS? -> \n");
	     dummy[i]->grunt.abilities.wis=(get_int(1,25,FALSE));
	     printf("\nWIS set to %d.",dummy[i]->grunt.abilities.wis);
	     break;
           case 4:
	     printf("\nCurrent DEX is %d.",dummy[i]->grunt.abilities.dex);
	     printf("\nNew DEX? -> \n");
	     dummy[i]->grunt.abilities.dex=(get_int(1,25,FALSE));
	     printf("\nDEX set to %d.",dummy[i]->grunt.abilities.dex);
	     break;
           case 5:
	     printf("\nCurrent CON is %d.",dummy[i]->grunt.abilities.con);
	     printf("\nNew CON? -> ");
	     dummy[i]->grunt.abilities.con=(get_int(1,25,FALSE));
	     printf("\nCON set to %d.",dummy[i]->grunt.abilities.con);
	     break;
           case 6:
	     printf("\nCurrent CHA is %d.",dummy[i]->grunt.abilities.chr);
	     printf("\nWhat do you want to change Charisma for??\n");
	     printf("New CHA? -> ");
	     dummy[i]->grunt.abilities.chr=(get_int(1,25,FALSE));
	     printf("\nCHA set to %d.",dummy[i]->grunt.abilities.chr);
	     break;
           case 7:
	     printf("\nCurrent STR_add is %d.",
               dummy[i]->grunt.abilities.str_add);
	     printf("\nNew STR_add? -> \n");
	     dummy[i]->grunt.abilities.str_add=(get_int(0,99,TRUE));
	     printf("\nSTR_add set to %d.",dummy[i]->grunt.abilities.str_add);
	     break;
           default:
	     printf("\nFinished with Stats ... \n");
	     break;
