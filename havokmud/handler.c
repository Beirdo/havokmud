...
int isname(const char *str, const char *namelist)
{
  ...
  static char	buf[MAX_INPUT_LENGTH+40], names[MAX_INPUT_LENGTH+40], *s;
...
void char_to_room(struct char_data *ch, long room)
{
 ...
 // Add-on by Manwe Windmaster 120198
  if (IS_AFFECTED(ch, AFF_FIRESHIELD))
    if (rp->sector_type == SECT_UNDERWATER) {
      affect_from_char(ch, AFF_FIRESHIELD);
      //send_to_char("When it touches water, your fireshield is extinguished!\n\r", ch);
    }
 // End of add-on
