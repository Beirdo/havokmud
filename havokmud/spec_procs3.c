    ...
    if (cmd == 72) {
        ...
        if (GetMaxLevel(ch) < 99) {
            if (obj_index[obj->item_number].virtual != 
                ...
                /*
                 * if(!(strcmp(questwinner,GET_NAME(ch)))) { questwon++;
                 * ch->specials.questwon = ch->specials.questwon + 1; }
                 * else { sprintf(questwinner,"%s",GET_NAME(ch)); questwon 
                 * = 0; } 
                 */
                ch->specials.questwon = ch->specials.questwon + 1;
                ...
                if (!obj2) {
                    log("/* SHit.. error in questor mob.. Tell banon or make "
                        "the items.*/");
                ...
                // do_say(vict, "Hey.. I got something else for you to get 
                // too.",0);
                time_diff = 201;
            }
            return (TRUE);
    ...
    if (cmd == 72) {
        ...
        if (obj_index[obj->item_number].virtual != universal) {
            ...
            if (obj2 = read_object(universal + 1, VIRTUAL)) {
                ...
                // act("$n puts $p into $s
                // pocket.",TRUE,vict,obj,0,TO_ROOM);
                obj_from_char(obj);
                extract_obj(obj);
                return (TRUE);
            } else {
        ...
        if (obj != get_obj_in_list_vis(ch, "head of Matron Singh",
            ...
            if (tgt->equipment[WEAR_HEAD]) {
                sprintf(buf, "Ah, she's dead, is she? Serves her right. "
                             ...
                             "my people. Perhaps you would be a better choice.",
                        GET_NAME(ch));
                do_say(tgt, buf, 19);
                ...
                sprintf(buf, "Hrm, you killed her, eh? Well, I don't lead "
                             ...
                             "leader, and leave me alone.", GET_NAME(ch));
                do_say(tgt, buf, 19);
                return (TRUE);
            }
        }
...
int guardian_sin(struct char_data *ch, struct char_data *vict)
{
    ...
    case 1:
        ...
        break;
    case 2:
        // if (IS_NPC(ch)) /* since slow doesn't really affect mobs, check 
