	...
	if(obj) {
		if((obj->item_number > -1) && (cost->ok) && ItemEgoClash(ch,obj,0) > -5) {
			...
			if(re) {
				...
				/* Now fill in the original description, making sure not to overrun the buffer. */
...
{
	int discount = 0, nbdays, i, ii, limited_items=0, exit=0;
	char buf[MAX_INPUT_LENGTH];
	struct obj_data *tmp, *tmp_next_obj, *rare;
	...
	if (limited_items > MaxLimited(GetMaxLevel(ch))) {
		if (receptionist) {
			sprintf(buf,"$c0013[$c0015$n$c0013] tells you 'Sorry, but I can't store more than %d limited items.",MaxLimited(GetMaxLevel(ch)));
			act(buf,FALSE,receptionist,0,ch,TO_VICT);
			return(FALSE);
		}

		 /* auto renting an idle person, lets wack items they should not carry here! (limited items) */
	...
	if (cost->no_carried > MAX_OBJ_SAVE) {
		if (receptionist) {
			sprintf(buf,"$c0013[$c0015$n$c0013] tells you 'Sorry, but I can't store more than %d items.", MAX_OBJ_SAVE);
			act(buf,FALSE,receptionist,0,ch,TO_VICT);
		}
		return(FALSE);
	}
...
*/
	...
	sprintf(buf, "$c0013[$c0015$n$c0013] tells you 'It will cost you %d coins per day.'",cost->total_cost);
	act(buf,FALSE,receptionist,0,ch,TO_VICT);
...
#if 0
	/* this appears to fuck with saving for polies.. not exactly sure why.  (jdb) */
...
/* you made it back from the crash in time, 1 hour grace period. */
		...
		if (GET_GOLD(ch) < 0) {
			if(GET_BANK(ch)+GET_GOLD(ch) < 0) {
				...
				send_to_char("You don't even have enough in your bank account!!\n\r",ch);
				GET_GOLD(ch) = 0;
				GET_BANK(ch) = 0;
				found = FALSE;
			} else {
				...
				send_to_char("I'm going to have to charge you a little extra for that though.\n\r",ch);
