...
{
  char bigbuf[MAX_STRING_LENGTH], tmpbuf[64];
  int diff;
  ...
  if (d->showstr_page + 1 >= d->showstr_count) {
    SEND_TO_Q(ParseAnsiColors(IS_SET(d->character->player.user_flags,USE_ANSI),d->showstr_vector[d->showstr_page]), d);
    free(d->showstr_vector);
    d->showstr_count = 0;
    if (d->showstr_head) {
      free(d->showstr_head);
  ...
  /* Or if we have more to show.... */
    ...
    strncpy(bigbuf, d->showstr_vector[d->showstr_page], diff = ((int) d->showstr_vector[d->showstr_page + 1]) -
            ((int) d->showstr_vector[d->showstr_page]));
    bigbuf[diff] = '\0';

    SEND_TO_Q(ParseAnsiColors(IS_SET(d->character->player.user_flags,USE_ANSI),bigbuf), d);
    d->showstr_page++;
  }
}
...
/*
 * The following list_ functions are for building a very large dynamic string
 * buffer which is attached the a player's descriptor and then sending it out to
 * the character via page_string.
 */

/*
 * Initialize the list_string pointer on the character's descriptor.
 */
void list_init(struct descriptor_data *d)
{
  if (d->list_string != NULL)
   // mprintf(line_log, ONE_LINE, LOG_BEEP | LOG_ASSERT, SEV_ALL,
   //         "list_init called for %s when d->list_string already allocated", d->account.account_name);
...
{
  va_list  va;
  int      new_size, slen, tlen;
  char     listbuf[MAX_STRING_LENGTH], *new_list;

  if (d->list_string == NULL)
    //mprintf(line_log, ONE_LINE, LOG_BEEP | LOG_ASSERT, SEV_ALL,
    //        "list_append called for %s before list_init", d->account.account_name);
...
void list_end(struct descriptor_data *d)
{
  if (d->list_string == NULL)
    //mprintf(line_log, ONE_LINE, LOG_BEEP | LOG_ASSERT, SEV_ALL,
    //        "list_end called for %s before list_init", d->account.account_name);
