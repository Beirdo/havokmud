...
{
  int door;
  char type[MAX_INPUT_LENGTH], dir[MAX_INPUT_LENGTH], buf[MAX_STRING_LENGTH+40];
  struct room_direction_data    *exitp,*back;
  struct room_data      *rp;
...
{
  int door;
  char type[MAX_INPUT_LENGTH], dir[MAX_INPUT_LENGTH], buf[MAX_STRING_LENGTH+60];
...
if(IS_AFFECTED(ch,AFF_FLYING))
   ...
   }else /*if character isn't flying...*/
  ...
  if(ch->equipment[i])
   for(j = 0;j < MAX_OBJ_AFFECT; j++)
      {if(ch->equipment[i]->affected[j].location == APPLY_SPELL &&
         ...
         /*Search for fly affect in all worn items
           Messy but I think it will work*/
      return(TRUE);
      }
return(FALSE);
}
...
if(IS_AFFECTED(ch,AFF_FLYING))
  ...
  affect_from_char(ch, COND_WINGS_TIRED);
    send_to_char("You force your fatigued wings to lift you into the air.\n\r",ch);
    act("$n unfolds $s wings and takes flight!", TRUE, ch, 0, 0,
TO_ROOM);

  /*We add one to the tired amount on the theory that launching takes some
effort*/
...
TO_ROOM);
    ...
    act("$n leaps high into the air... and stays there.",TRUE, ch, 0, 0, TO_ROOM);
